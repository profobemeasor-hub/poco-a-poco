...
function AICoach({lp}){
  ...
  const callAI=async(userText)=>{
    if(!AI_WORKER_URL){setError('Live AI is not configured yet. Run SETUP_AI_COACH.command on your Mac.');return}
  ...
  const savePin=()=>{if(pin.trim()){localStorage.setItem('poco-ai-pin',pin.trim());setShowPin(false);setError('')}};

  if(offline)return <><Header title="Offline Roleplay" sub="Works without internet" onBack={()=>setOffline(false)}/><OfflineRoleplay lp={lp}/></>;

  return <><Header title="AI Coach" sub="Live everyday conversation"/><div style={{padding:'0 20px 28px'}}>
    <div style={{...card,padding:14,marginBottom:12,background:T.raised}}><div style={{fontFamily:mono,fontSize:9,color:T.jade,letterSpacing:'.12em'}}>EVERYDAY FIRST</div><div style={{fontSize:13.5,color:T.sand,lineHeight:1.55,marginTop:7}}>Live conversation stays outside work: daily life, social situations, errands, health, transport and Guatemala conversation.</div></div>
    ...
    {showPin&&<div style={{...card,padding:17,marginBottom:12,borderColor:T.marigold}}><div style={{fontFamily:serif,fontSize:20}}>Your AI PIN</div><div style={{fontSize:12.5,color:T.sand,lineHeight:1.5,marginTop:5}}>Enter the private PIN you set when deploying the AI gateway. It stays on this device.</div><input type="password" value={pin} onChange={e=>setPin(e.target.value)} placeholder="Private PIN" style={{width:'100%',boxSizing:'border-box',marginTop:12,padding:12,borderRadius:12,border:`1px solid ${T.line}`,background:T.ground,color:T.cream,fontSize:16}}/><button onClick={savePin} style={{width:'100%',marginTop:9,padding:12,borderRadius:12,border:0,background:T.marigold,color:T.ground,fontWeight:750}}>Save PIN</button></div>}
    {messages.length===0?<div style={{...card,padding:22,textAlign:'center'}}><Sparkles size={30} color={T.marigold} style={{margin:'0 auto 12px'}}/><div style={{fontFamily:serif,fontSize:26}}>Have a real conversation.</div><div style={{fontSize:13.5,color:T.sand,lineHeight:1.55,marginTop:8}}>The coach will respond naturally, correct only what matters, and keep the conversation moving.</div><button onClick={start} disabled={busy} style={{width:'100%',marginTop:18,padding:14,borderRadius:14,border:0,background:T.jade,color:T.ground,fontWeight:750}}>{busy?'Starting…':'Start conversation'}</button><button onClick={()=>setOffline(true)} style={{width:'100%',marginTop:9,padding:12,borderRadius:14,border:`1px solid ${T.line}`,background:'transparent',color:T.sand}}>Use offline roleplay</button></div>:
