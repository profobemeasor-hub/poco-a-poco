...
function OfflineRoleplay({lp}){const [id,setId]=useState(null);const [answer,setAnswer]=useState('');const [result,setResult]=useState(null);const [listening,setListening]=useState(false);const recognition=useRef(null);const s=SCENARIOS.find(x=>x.id===id);const character=s&&CHARACTERS.find(c=>c.id===s.character);
...
function AICoach({lp}){
  ...
  const [pin,setPin]=useState(()=>localStorage.getItem('poco-ai-pin')||'');
  const [showPin,setShowPin]=useState(!localStorage.getItem('poco-ai-pin'));
  ...
  const starters={
    everyday:'Start a natural everyday conversation with me in Guatemala. Pick a normal situation such as coffee, shopping, neighbours, transport or weekend plans.',
    social:'Start a friendly social conversation as if we have just met at a café or casual gathering in Guatemala.',
    practical:'Start a practical roleplay involving an apartment issue, bank, pharmacy, doctor, directions or buying something.',
    spanish:'Speak to me almost entirely in Spanish. Keep it natural and suitable for my level.'
  };

  const loadMemory=async(activePin=pin)=>{
    if(!AI_WORKER_URL||!activePin)return;
    try{
      const r=await fetch(`${AI_WORKER_URL}/memory`,{headers:{'X-Poco-Pin':activePin}});
      const data=await r.json().catch(()=>({}));
      if(r.ok)setMemory(data.memory||null);
    }catch{}
  };

  useEffect(()=>{if(pin&&!showPin)loadMemory(pin)},[]); // eslint-disable-line react-hooks/exhaustive-deps

  const listen=()=>{const SR=window.SpeechRecognition||window.webkitSpeechRecognition;if(!SR){setError('Speech recognition is not available here. Type your answer instead.');return}const r=new SR();r.lang='es-GT';r.interimResults=false;r.onstart=()=>setListening(true);r.onend=()=>setListening(false);r.onerror=()=>setListening(false);r.onresult=e=>setText(e.results[0][0].transcript);r.start()};

  const callAI=async(userText)=>{
    if(!AI_WORKER_URL){setError('Live AI is not configured yet.');return}
    if(!pin){setShowPin(true);return}
    setBusy(true);setError('');
    const next=[...messages,{role:'user',content:userText}];setMessages(next);setText('');
    try{
      const r=await fetch(`${AI_WORKER_URL}/chat`,{method:'POST',headers:{'Content-Type':'application/json','X-Poco-Pin':pin},body:JSON.stringify({messages:next.slice(-12),mode,confidence:lp.state.confidence})});
      const data=await r.json().catch(()=>({}));
      if(!r.ok) throw new Error(data.error||`AI request failed (${r.status})`);
      const assistant={role:'assistant',content:data.reply||'',correction:data.correction||'',tip:data.tip||'',vocabulary:data.vocabulary||[]};
      setMessages(m=>[...m,assistant]);
      if(data.memory)setMemory(data.memory);
      if(data.reply) say(data.reply,.88);
      if(data.score!=null) lp.recordScenario(`ai:${mode}`, 'social', Number(data.score)||70);
    }catch(e){setError(e.message||'Could not reach the AI coach.');}
    finally{setBusy(false)}
  };

  const start=()=>callAI(starters[mode]);
  const send=()=>{if(text.trim()&&!busy)callAI(text.trim())};
  const savePin=()=>{if(pin.trim()){localStorage.setItem('poco-ai-pin',pin.trim());setShowPin(false);setError('');setTimeout(()=>loadMemory(pin.trim()),0)}};

  if(offline)return <><Header title="Offline Roleplay" sub="Works without internet" onBack={()=>setOffline(false)}/><OfflineRoleplay lp={lp}/></>;

  return <><Header title="AI Coach" sub="Now remembers your learning"/><div style={{padding:'0 20px 28px'}}>
    <div style={{...card,padding:14,marginBottom:12,background:T.raised}}><div style={{fontFamily:mono,fontSize:9,color:T.jade,letterSpacing:'.12em'}}>MEMORY ON</div><div style={{fontSize:13.5,color:T.sand,lineHeight:1.55,marginTop:7}}>{memory?.sessions?`Your coach has ${memory.sessions} saved practice turns and will subtly revisit weak areas.`:'The coach will start building a private learning memory as you practice.'}</div>{memory?.averageScore!=null&&<div style={{fontFamily:serif,fontSize:24,color:T.marigold,marginTop:8}}>{memory.averageScore}% <span style={{fontFamily:'inherit',fontSize:13,color:T.sand}}>average communication</span></div>}</div>
    <div style={{display:'flex',gap:7,overflowX:'auto',paddingBottom:10}}>{[['everyday','Daily life'],['social','Social'],['practical','Practical'],['spanish','Mostly Spanish']].map(([id,label])=><button key={id} onClick={()=>{setMode(id);setMessages([])}} style={{whiteSpace:'nowrap',padding:'9px 12px',borderRadius:99,border:`1px solid ${mode===id?T.marigold:T.line}`,background:mode===id?T.marigold:T.surface,color:mode===id?T.ground:T.sand,fontFamily:mono,fontSize:9.5}}>{label.toUpperCase()}</button>)}</div>
    {showPin&&<div style={{...card,padding:17,marginBottom:12,borderColor:T.marigold}}><div style={{fontFamily:serif,fontSize:20}}>Your AI PIN</div><div style={{fontSize:12.5,color:T.sand,lineHeight:1.5,marginTop:5}}>Enter your private AI PIN. It stays on this device.</div><input type="password" value={pin} onChange={e=>setPin(e.target.value)} placeholder="Private PIN" style={{width:'100%',boxSizing:'border-box',marginTop:12,padding:12,borderRadius:12,border:`1px solid ${T.line}`,background:T.ground,color:T.cream,fontSize:16}}/><button onClick={savePin} style={{width:'100%',marginTop:9,padding:12,borderRadius:12,border:0,background:T.marigold,color:T.ground,fontWeight:750}}>Save PIN</button></div>}
    {messages.length===0?<div style={{...card,padding:22,textAlign:'center'}}><Sparkles size={30} color={T.marigold} style={{margin:'0 auto 12px'}}/><div style={{fontFamily:serif,fontSize:26}}>Continue where you left off.</div><div style={{fontSize:13.5,color:T.sand,lineHeight:1.55,marginTop:8}}>The tutor now uses your previous corrections, vocabulary and communication scores to shape the next conversation.</div><button onClick={start} disabled={busy} style={{width:'100%',marginTop:18,padding:14,borderRadius:14,border:0,background:T.jade,color:T.ground,fontWeight:750}}>{busy?'Starting…':'Start conversation'}</button><button onClick={()=>setOffline(true)} style={{width:'100%',marginTop:9,padding:12,borderRadius:14,border:`1px solid ${T.line}`,background:'transparent',color:T.sand}}>Use offline roleplay</button></div>:
    <div><div style={{display:'flex',flexDirection:'column',gap:9,marginBottom:12}}>{messages.map((m,i)=><div key={i} style={{alignSelf:m.role==='user'?'flex-end':'flex-start',maxWidth:'88%',padding:'12px 14px',borderRadius:16,background:m.role==='user'?T.raised:T.surface,border:`1px solid ${m.role==='user'?T.jade:T.line}`}}><div style={{fontFamily:m.role==='assistant'?serif:'inherit',fontSize:m.role==='assistant'?17:15,lineHeight:1.45}}>{m.content}</div>{m.role==='assistant'&&m.correction&&<div style={{marginTop:9,paddingTop:9,borderTop:`1px solid ${T.line}`,fontSize:12.5,color:T.marigold}}><b>Correction:</b> {m.correction}</div>}{m.role==='assistant'&&m.tip&&<div style={{marginTop:5,fontSize:12.5,color:T.sand}}>{m.tip}</div>}{m.role==='assistant'&&<button onClick={()=>say(m.content,.86)} style={{...btnIcon,width:32,height:32,marginTop:9}}><Volume2 size={15}/></button>}</div>)}</div>
    <div style={{...card,padding:12,position:'sticky',bottom:82}}><textarea value={text} onChange={e=>setText(e.target.value)} placeholder="Answer in Spanish…" style={{width:'100%',boxSizing:'border-box',minHeight:74,padding:11,borderRadius:11,border:`1px solid ${T.line}`,background:T.ground,color:T.cream,fontSize:16,resize:'none'}}/><div style={{display:'flex',gap:8,marginTop:8}}><button onClick={listen} style={{flex:1,padding:12,borderRadius:12,border:`1px solid ${listening?T.rose:T.line}`,background:'transparent',color:listening?T.rose:T.cream}}><Mic size={16} style={{verticalAlign:'middle',marginRight:5}}/>{listening?'Listening…':'Speak'}</button><button onClick={send} disabled={!text.trim()||busy} style={{flex:1,padding:12,borderRadius:12,border:0,background:text.trim()&&!busy?T.jade:T.surface,color:text.trim()&&!busy?T.ground:T.sand,fontWeight:750}}><Send size={16} style={{verticalAlign:'middle',marginRight:5}}/>{busy?'Thinking…':'Send'}</button></div></div></div>}
    {memory?.weakCorrections?.length>0&&<div style={{...card,padding:15,marginTop:12}}><div style={{fontFamily:mono,fontSize:9,color:T.marigold,letterSpacing:'.11em'}}>COACH REMEMBERS</div>{memory.weakCorrections.slice(0,3).map(x=><div key={x.text} style={{fontSize:12.5,color:T.sand,marginTop:7}}>• {x.text}</div>)}</div>}
    {error&&<div style={{marginTop:10,padding:11,borderRadius:12,border:`1px solid ${T.rose}`,color:T.rose,fontSize:12.5}}>{error}</div>}
  </div></>;
}
...
function Progress({lp}){
  ...
  const pin=()=>localStorage.getItem('poco-ai-pin')||'';
  const load=async()=>{if(!AI_WORKER_URL||!pin())return;try{const r=await fetch(`${AI_WORKER_URL}/memory`,{headers:{'X-Poco-Pin':pin()}});const d=await r.json();if(r.ok)setMemory(d.memory);else setMemoryError(d.error||'Could not load AI memory.')}catch{setMemoryError('Could not load AI memory.')}};
  useEffect(()=>{load()},[]); // eslint-disable-line react-hooks/exhaustive-deps
  const resetMemory=async()=>{try{const r=await fetch(`${AI_WORKER_URL}/memory/reset`,{method:'POST',headers:{'X-Poco-Pin':pin()}});if(!r.ok)throw new Error();setMemory({sessions:0,averageScore:null,weakCorrections:[],vocabulary:[],topics:[],recent:[]});setConfirmReset(false)}catch{setMemoryError('Could not reset AI memory.')}};
  return <><Header title="Confidence" sub="Real life + AI memory"/><div style={{padding:'0 20px 28px'}}>
    <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10,marginBottom:18}}><div style={{...card,padding:15}}><div style={{fontFamily:mono,fontSize:9,color:T.sand}}>XP</div><div style={{fontFamily:serif,fontSize:29,marginTop:4}}>{lp.state.xp}</div></div><div style={{...card,padding:15}}><div style={{fontFamily:mono,fontSize:9,color:T.sand}}>AI MEMORY</div><div style={{fontFamily:serif,fontSize:29,marginTop:4}}>{memory?.sessions??'—'}</div></div></div>
    {memory&&<div style={{...card,padding:17,marginBottom:18,borderColor:T.violet}}><div style={{display:'flex',justifyContent:'space-between',alignItems:'baseline'}}><div style={{fontFamily:serif,fontSize:21}}>What your coach remembers</div>{memory.averageScore!=null&&<div style={{fontFamily:serif,fontSize:25,color:T.violet}}>{memory.averageScore}%</div>}</div><div style={{fontSize:12.5,color:T.sand,lineHeight:1.5,marginTop:5}}>Stored privately in your Cloudflare D1 learning memory.</div>{memory.weakCorrections?.length>0&&<><div style={{fontFamily:mono,fontSize:9,color:T.marigold,marginTop:15}}>RECURRING CORRECTIONS</div>{memory.weakCorrections.slice(0,4).map(x=><div key={x.text} style={{fontSize:12.5,color:T.sand,marginTop:6}}>• {x.text}</div>)}</>}{memory.vocabulary?.length>0&&<><div style={{fontFamily:mono,fontSize:9,color:T.jade,marginTop:15}}>WORDS TO REUSE</div><div style={{display:'flex',flexWrap:'wrap',gap:6,marginTop:7}}>{memory.vocabulary.slice(0,8).map(x=><span key={x.text} style={{fontSize:12,padding:'6px 8px',borderRadius:99,background:T.raised,color:T.cream}}>{x.text}</span>)}</div></>}{confirmReset?<div style={{marginTop:16,paddingTop:13,borderTop:`1px solid ${T.line}`}}><div style={{fontSize:12.5,color:T.rose}}>This deletes the AI conversation memory stored in D1. Your local missions and confidence remain.</div><div style={{display:'flex',gap:7,marginTop:9}}><button onClick={()=>setConfirmReset(false)} style={{flex:1,padding:10,borderRadius:10,border:`1px solid ${T.line}`,background:'transparent',color:T.cream}}>Cancel</button><button onClick={resetMemory} style={{flex:1,padding:10,borderRadius:10,border:0,background:T.rose,color:T.ground,fontWeight:700}}>Delete memory</button></div></div>:<button onClick={()=>setConfirmReset(true)} style={{marginTop:15,padding:9,borderRadius:10,border:`1px solid ${T.line}`,background:'transparent',color:T.sand,fontSize:12}}>Reset AI memory</button>}</div>}
    {memoryError&&<div style={{color:T.rose,fontSize:12.5,marginBottom:12}}>{memoryError}</div>}
    <div style={{...card,padding:17,marginBottom:18}}>{Object.entries(lp.state.confidence).map(([k,v])=><div key={k} style={{marginBottom:13}}><div style={{display:'flex',justifyContent:'space-between',fontSize:13,marginBottom:6}}><span>{CONFIDENCE_LABELS[k]}</span><span style={{fontFamily:mono,color:v>=70?T.jade:v>=40?T.marigold:T.rose}}>{v}%</span></div><ProgressBar value={v} color={v>=70?T.jade:v>=40?T.marigold:T.rose}/></div>)}</div>
    <div style={{...card,padding:17}}><div style={{fontFamily:serif,fontSize:20}}>Daily reflection</div><div style={{fontSize:13,color:T.sand,lineHeight:1.5,marginTop:5}}>What Spanish did you use today? Write one sentence.</div><textarea value={reflection} onChange={e=>setReflection(e.target.value)} placeholder="Hoy hablé con…" style={{width:'100%',boxSizing:'border-box',minHeight:82,marginTop:12,padding:12,borderRadius:12,border:`1px solid ${T.line}`,background:T.ground,color:T.cream}}/><button onClick={()=>{lp.addReflection(reflection);setReflection('')}} disabled={!reflection.trim()} style={{width:'100%',padding:12,marginTop:8,borderRadius:12,border:0,background:reflection.trim()?T.marigold:T.surface,color:reflection.trim()?T.ground:T.sand,fontWeight:700}}>Save reflection</button>{lp.state.reflections.slice(0,3).map(r=><div key={r.at} style={{fontSize:13,color:T.sand,paddingTop:10,marginTop:10,borderTop:`1px solid ${T.line}`}}>{r.text}</div>)}</div>
  </div></>;
}
