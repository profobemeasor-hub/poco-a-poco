...
function SmartCoach({lp}){const [id,setId]=useState(null);const [answer,setAnswer]=useState('');const [result,setResult]=useState(null);const [listening,setListening]=useState(false);const recognition=useRef(null);const s=SCENARIOS.find(x=>x.id===id);const character=s&&CHARACTERS.find(c=>c.id===s.character);
...
function Progress({lp}){const [reflection,setReflection]=useState('');return <><Header title="Confidence" sub="Stored privately on this device"/><div style={{padding:'0 20px 28px'}}><div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10,marginBottom:18}}><div style={{...card,padding:15}}><div style={{fontFamily:mono,fontSize:9,color:T.sand}}>XP</div><div style={{fontFamily:serif,fontSize:29,marginTop:4}}>{lp.state.xp}</div></div><div style={{...card,padding:15}}><div style={{fontFamily:mono,fontSize:9,color:T.sand}}>MISSIONS DONE</div><div style={{fontFamily:serif,fontSize:29,marginTop:4}}>{Object.keys(lp.state.missionDone).length}</div></div></div><div style={{...card,padding:17,marginBottom:18,borderColor:T.violet}}><div style={{fontFamily:serif,fontSize:20}}>What Smart Coach remembers</div><div style={{fontSize:12.5,color:T.sand,lineHeight:1.5,marginTop:5}}>Your roleplay scores, confidence, missions and reflections stay on this device. No API account or cloud AI is required.</div><div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8,marginTop:12}}><div style={{background:T.raised,borderRadius:12,padding:11}}><div style={{fontFamily:mono,fontSize:8.5,color:T.sand}}>PRACTICE ROUNDS</div><div style={{fontFamily:serif,fontSize:24,marginTop:4}}>{Object.values(lp.state.scenarioScores).reduce((n,a)=>n+a.length,0)}</div></div><div style={{background:T.raised,borderRadius:12,padding:11}}><div style={{fontFamily:mono,fontSize:8.5,color:T.sand}}>REFLECTIONS</div><div style={{fontFamily:serif,fontSize:24,marginTop:4}}>{lp.state.reflections.length}</div></div></div></div><div style={{...card,padding:17,marginBottom:18}}>{Object.entries(lp.state.confidence).map(([k,v])=><div key={k} style={{marginBottom:13}}><div style={{display:'flex',justifyContent:'space-between',fontSize:13,marginBottom:6}}><span>{CONFIDENCE_LABELS[k]}</span><span style={{fontFamily:mono,color:v>=70?T.jade:v>=40?T.marigold:T.rose}}>{v}%</span></div><ProgressBar value={v} color={v>=70?T.jade:v>=40?T.marigold:T.rose}/></div>)}</div><div style={{...card,padding:17}}><div style={{fontFamily:serif,fontSize:20}}>Daily reflection</div><div style={{fontSize:13,color:T.sand,lineHeight:1.5,marginTop:5}}>What Spanish did you use today? Write one sentence. This keeps the app connected to your real life.</div><textarea value={reflection} onChange={e=>setReflection(e.target.value)} placeholder="Hoy hablé con…" style={{width:'100%',boxSizing:'border-box',minHeight:82,marginTop:12,padding:12,borderRadius:12,border:`1px solid ${T.line}`,background:T.ground,color:T.cream}}/><button onClick={()=>{lp.addReflection(reflection);setReflection('')}} disabled={!reflection.trim()} style={{width:'100%',padding:12,marginTop:8,borderRadius:12,border:0,background:reflection.trim()?T.marigold:T.surface,color:reflection.trim()?T.ground:T.sand,fontWeight:700}}>Save reflection</button>{lp.state.reflections.slice(0,3).map(r=><div key={r.at} style={{fontSize:13,color:T.sand,paddingTop:10,marginTop:10,borderTop:`1px solid ${T.line}`}}>{r.text}</div>)}</div></div></>}
